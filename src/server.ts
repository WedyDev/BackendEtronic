...
import { QuestionRoomService } from './services/QuestionRoomService';
...
const questionRoomService = new QuestionRoomService();
...
io.on('connection', (socket) => {
  ...
  socket.on('user:join', (username: string, roomCode: string, callback?: (response: User | { error: string }) => void) => {
    const sendJoinResponse = (response: User | { error: string }) => {
  ...
  const emitQuestions = (roomCode: string) => {
  ...
  socket.on('question:add', (text: string, callback?: (response: { error?: string }) => void) => {
    const sendResponse = (response: { error?: string }) => {
      if (typeof callback === 'function') {
        callback(response);
      }
    };
  ...
  socket.on('question:answer', (payload: { questionId: string; text: string }, callback?: (response: { error?: string }) => void) => {
    const sendResponse = (response: { error?: string }) => {
      if (typeof callback === 'function') {
        callback(response);
      }
  ...
  socket.on('question:voteSkip', (questionId: string, callback?: (response: { error?: string; skipped?: boolean }) => void) => {
    const sendResponse = (response: { error?: string; skipped?: boolean }) => {
      if (typeof callback === 'function') {
        callback(response);
      }
    };
    ...
    const result = questionRoomService.voteSkip(
      user.roomCode,
      questionId,
      user.id,
      userService.getUserCount(user.roomCode)
    );
  ...
  socket.on('drawing:action', (action: any) => {
    const user = userService.getUserBySocketId(socket.id);
    if (!user) return;
    ...
    console.log(
      `[DIBUJO] ${user.username} - Sala: ${user.roomCode} - Tipo: ${action.type} - ID: ${drawingAction.id}`
    );
  });

  // Evento: Mensaje de chat
  socket.on('chat:send', (messageText: string) => {
    const user = userService.getUserBySocketId(socket.id);
    if (!user) return;
    ...
    console.log(
      `[MENSAJE] ${user.username} (${user.roomCode}): ${messageText.substring(0, 50)}`
    );
  });
